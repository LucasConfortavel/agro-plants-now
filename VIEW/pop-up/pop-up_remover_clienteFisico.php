<?php
    if(isset($_POST['remover'])){
        header("location:../../VIEW/adm/vendas-adm.php");
    }
?>



<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cadastro de Venda - PF</title>
    <link rel="stylesheet" href="../../PUBLIC/css/pop-up-cadastroVenda.css">
    <link rel="stylesheet" href="../../PUBLIC/css/style.css">
</head>
<body>
    <div class="ym_pop-up">
        
        <div class="ym_area-titulo">
            <h1 class="ym_titulo">Remover cliente</h1>
        </div>

        <div class="ym_superior-pop-up">                                
            <a class="ym_pop-up-select ym_pessoa-fisica">Pessoa Física</a>
            <a class="ym_pop-up-select ym_pessoa-juridica" onclick="abrirPopup('../../VIEW/pop-up/pop-up_remover_clienteJuridico.php')" style="background-color: #c9c9c9; border-left: 2px solid rgb(190, 190, 190); border-bottom: 2px solid rgb(190, 190, 190); border-radius: 0px 0px 0px 10px; cursor: pointer; ">Pessoa Jurídica</a>
        </div>

        <form action="" method="post" class="ym_form-pop-up">

            <div class="ym_area-input">
                <p class="ym_input-aviso-obrigatorio">*obrigatório</p>
                <input class="ym_pop-up-input" name="cpf" type="text" placeholder="CPF" oninput="ym_Validacao()" >
            </div>                               

            <div class="ym_area-btn-submit">
                <input class="ym_btn-remover" name="remover" type="submit" value="Remover">
            </div>


        </form>

    </div>


<script>

    function ym_Validacao(){
        const ym_inputs = document.querySelectorAll(".ym_pop-up-input");
        const ym_msg = document.querySelectorAll(".ym_input-aviso-obrigatorio");
        
        if(ym_inputs[0].value.length == 0){
            setError(0);
        }
        else{
            removeError(0);
        }
        function setError(index){
            ym_msg[index].style.display = "block";
        }

        function removeError(index){
            ym_msg[index].style.display='none';
        }

    }

</script>
</body>
</html>